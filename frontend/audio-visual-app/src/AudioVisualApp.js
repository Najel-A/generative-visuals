...
// Component for displaying audio analysis results
const AudioAnalysisResults = ({ audioFeatures }) => (
...
// Component for each processing stage
...
// Component for displaying the overall processing status
const ProcessingStatus = ({ processingStages }) => (
  <div className="absolute bottom-5 left-1/2 transform -translate-x-1/2 w-full max-w-3xl rounded-lg bg-gray-800/60 p-3 shadow-lg backdrop-blur-lg">
    ...
    <div className="grid grid-cols-3 gap-6">
      <ProcessingStage title="Audio Analysis" icon={Music} {...processingStages.audioAnalysis} />
      <ProcessingStage title="Beat Detection" icon={Sparkles} {...processingStages.beatDetection} />
      <ProcessingStage title="Visual Generation" icon={Video} {...processingStages.visualGeneration} />
    </div>
  </div>
);

// Component for displaying features on the landing page
...
// Component for the main processing page layout
const ProcessingPage = ({ audioFeatures, processingStages, goBack }) => (
...
const LandingPage = ({ handleFileSelect, handleProcess, selectedFile, error, isProcessing }) => (
...
const AudioVisualApp = () => {
  ...
  return isProcessingPage ? (
    <ProcessingPage audioFeatures={audioFeatures} processingStages={processingStages} goBack={goBack} />
  ) : (
    <LandingPage 
